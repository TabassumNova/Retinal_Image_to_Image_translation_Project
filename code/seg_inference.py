...
class SegRetino(object):
    ...
    def __init__(self, img_path="",original_image_path= False, original_image=0, size = (512, 512)):
    ...
    def inference(self, set_weight_dir = 'unet.pth', out_path = True, path = 'output.png', blend = True, blend_path = 'blend.png'):

        set_weight_dir = __PREFIX__ + "/seg_weights/" + set_weight_dir
        ...
        if self.original_image_path:
            ...
            x = x.to(self.device)
        ### only use it for predicting high resolution images
        # prev_w = image.shape[0]
        # prev_h = image.shape[1]
        # w = int(image.shape[0] * 0.2)
        ...
        with torch.no_grad():
            ...
            # pred_y = pred_y[0].to(self.device) ## (1, 512, 512)
        ...
        # line = np.ones((image.shape[1], 10, 3)) * 128

        # cat_images = np.concatenate(
        #     [line, pred_y * 255], axis=1
        # )

        # image = cv2.resize(pred_y, (prev_w, prev_h))

        # cv2.imwrite(path, cat_images)
        if out_path == True:
            cv2.imwrite(path, pred_y*255)
        else:
            return torch.from_numpy(pred_y)
